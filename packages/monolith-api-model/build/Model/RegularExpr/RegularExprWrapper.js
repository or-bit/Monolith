...
class RegularExprWrapper extends RegExp {
    ...
    static compose(string, quantifier, openBracket, closeBracket, regExp) {
        ...
        } else if (RegularExprWrapper.suffixQuantifiers.indexOf(quantifier) !== -1 || /{[1-9]+(,[1-9]*)?}/.test(quantifier)) {
            rExpr += quantifier;
        }
        return rExpr;
    }
    ...
    findAnyOf(string, quantifier) {
        if (Array.isArray(string)) {
            for (let i = 0; i < string.length; i += 1) {
                this.regExp = RegularExprWrapper.compose(RegularExprWrapper.quote(string[i]), quantifier, '[', ']', this.regExp);
            }
        } else {
            this.regExp = RegularExprWrapper.compose(RegularExprWrapper.quote(string), quantifier, '[', ']', this.regExp);
        }
        return this;
    }
    ...
    findAnyBetween(from, to, quantifier) {
        ...
        this.regExp = RegularExprWrapper.compose(`${fromRegEx}-${toRegEx}`, quantifier, '[', ']', this.regExp);
        return this;
    }
    ...
    findAnyAlternative(alternatives, quantifier) {
        ...
        this.regExp = RegularExprWrapper.compose(alternatives.join('|'), quantifier, '(', ')', this.regExp);
        return this;
    }
    ...
    findAllOf(strings) {
        if (Array.isArray(strings)) {
            for (let i = 0; i < strings.length; i += 1) {
                this.regExp = RegularExprWrapper.compose(RegularExprWrapper.quote(strings[i]), '+', '(', ')', this.regExp);
            }
        } else {
            this.regExp = RegularExprWrapper.compose(RegularExprWrapper.quote(strings), '+', '(', ')', this.regExp);
        }
        return this;
    }
