...
describe('RegularExp', () => {
    describe('#constructors', () => {
        it('should create default regexp using no args constructor', () => {
            ...
            expect(expr.toString()).to.equal(RegularExp.defaultRegExp.toString());
            console.log(expr.modifiers);
            expect(expr.modifiers.i).to.equal(expr.modifiers.g).to.equal(expr.modifiers.m).to.equal(false);
        });
        it('should create regexp using 1 argument', () => {
            const expr = new RegularExp(/[a-z]/gi);
            expect(expr).to.have.property('mod');
            expect(expr.toString()).to.equal('[a-z]');
            expect(expr.modifiers.i).to.equal(expr.modifiers.g).to.equal(true);
